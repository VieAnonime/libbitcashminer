...
std::vector<bitcash::GPUInfo> GPUsInfo();
...
std::vector<bitcash::GPUInfo> GPUsInfo(){
    return std::vector<bitcash::GPUInfo>();
};
...
namespace bitcash
{
    namespace miner
    {
        ...
        std::vector<bitcash::GPUInfo> GPUInfo()
        {
            return ::GPUsInfo();
        }
...
std::cout << "HASH: " << hex_header_hash << std::endl;
...
//                    be32enc(&bwork[i], work->data[i]);
                   ...
                   std::cout << std::hex << std::setfill('0') << std::setw(8) << bwork[i] << " ";
                }
    std::cout << std::endl;
